...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
	...
	\path (A) edge				node {$a\rightarrow a', D$} (B)
			  edge				node {$\begin{matrix}b\rightarrow b, D\\b'\rightarrow b', D\end{matrix}$} (D)
		  ...
		  (D) edge [loop below] node {$\begin{matrix}b\rightarrow b, D\\b'\rightarrow b', D\end{matrix}$} (D)
			  edge				node [swap, pos=0.25]{\textvisiblespace$\rightarrow$\textvisiblespace, $D$} (F)
